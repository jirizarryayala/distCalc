...
def findSpecIndex(preprocessedMZMLFile, scanNumber):
    optScanNum=scanNumber-1
    internalSeries=pd.Series(preprocessedMZMLFile.scan_id)
    index=abs(internalSeries-optScanNum).idxmin()
    return index
...
def distComp(preprocessedMZMLFile, specIndex, precursorMZR, charge, secondPreprocessedMZMLFile=None, thirdPreprocessedMZMLFile=None, autoQC=True):
...
def distQC(distributionClassObject):
         minInt=min(distributionClassObject.dataFrame.Intensity)
         QCindex=abs(minInt-distributionClassObject.dataFrame.Intensity).idxmin()
         y1=0
         y2=0
         for x in range(QCindex):
             if distributionClassObject.dataFrame.Intensity[x] > y1:
                 y1=distributionClassObject.dataFrame.Intensity[x]
                 firstIndex=abs(y1-distributionClassObject.dataFrame.Intensity).idxmin()
         for x in range(QCindex, len(distributionClassObject.dataFrame.Intensity)):
             if distributionClassObject.dataFrame.Intensity[x] > y2:
                 y2=distributionClassObject.dataFrame.Intensity[x]
                 secondIndex=abs(y2-distributionClassObject.dataFrame.Intensity).idxmin()
         if secondIndex-firstIndex>4:
             Intensity=[]
             mzr=[]
         else:
             return distributionClassObject
         for x in range(QCindex):
             Intensity.append(distributionClassObject.dataFrame.Intensity[x])
             mzr.append(distributionClassObject.dataFrame.mzr[x])
         dF=pd.DataFrame({"mzr": mzr, "Intensity":Intensity})
         newDist=distribution.distCompile(dF)
         return newDist